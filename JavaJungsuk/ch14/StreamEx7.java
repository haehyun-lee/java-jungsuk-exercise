...
public class StreamEx7 {
    public static void main(String[] args) {
        ...
        Map<Boolean, Optional<Student>> topScoreStudent = Stream.of(stuArr)
                .collect(Collectors.partitioningBy(Student::isMale,
                        Collectors.maxBy(Comparator.comparingInt(Student::getScore))));
        System.out.println("남학생 1등 : " + topScoreStudent.get(true).get());
        System.out.println("여학생 1등 : " + topScoreStudent.get(false).get());
        ...
        // 성별을 기준으로 분할된 Map을, 합격 기준으로 한번 더 분할 failedStuBySex.get(true) = Map<Boolean, List<Student>>
        Map<Boolean, Map<Boolean, List<Student>>> failedStuBySex = Stream.of(stuArr)
                .collect(Collectors.partitioningBy(Student::isMale,
                        Collectors.partitioningBy(s -> s.getScore() <= 100)));
        List<Student> failedMaleStu = failedStuBySex.get(true).get(true);       // 조건에 따른 true/false 구분 주의
